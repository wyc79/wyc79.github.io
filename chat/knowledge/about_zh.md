...
语料读起来都还算合理，只有实际校准数字才暴露问题。该问题此后已修复，中文 gate
现已重新启用。
...
## 关于你正在阅读的这个页面
link: index.html
访客常常会问自己正在阅读的这个页面：总结一下这个页面、这个页面讲了什么，或者介绍一下当前页面。本站每个项目页面都记录一件作品——它是什么、为什么做，以及如何实现。聊天助手知道访客正在看哪个页面，并会用该页面自身的内容来回答这类问题。
