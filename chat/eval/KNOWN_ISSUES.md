...
**Update (a later task): the "EN phenomenon" conclusion needed narrowing,
not overturning — two zh follow-up positives found and added.**
`FOLLOWUP_POSITIVES_PER_LANG` is now `{"en": 2, "zh": 2}`
(`followup-zh-01`/`followup-zh-02`, both in `golden.jsonl`). This does not
contradict the finding above; it narrows it.

This task re-swept with the *specific shape* of the EN winner
(`followup-en-01`, "what about tuning it": an elided argument plus a
dangling pronoun, near-zero content words) in mind, rather than the earlier
sweep's broader net. ~50 direct structural analogues of that shape —
那优化呢, 那这部分呢, 那性能呢, 那这个呢, 那怎么调, and further
deictic/task-request variants — were measured against the live zh gate
(threshold 0.3979) and **every one still leaks**, scoring 0.42–0.53
regardless of topic. This exactly reproduces the original finding for this
shape: it is not a fluke of the first sweep, and a rewriter targeting this
shape in zh still has no natural case to rescue.

But widening from that shape to plain **narrative-continuation fillers**
("and then?", "so, afterward?") — closely related to the original sweep's
own best two near-misses (`那后来呢` 0.3957 margin 0.0022, `再后来` 0.3768
margin 0.0211, both already noted above as "too thin to be robust") — found
several that clear the gate with real margin, all measured directly with
`rt.gate()`:

```
那再后来    0.3645   margin 0.0334   (best margin found)
再后来      0.3768   margin 0.0211   (same wording the original sweep already found,
                                      re-verified; this task's explicit ≥0.01 floor
                                      reverses the earlier "too thin" call for it)
那后来      0.3857   margin 0.0122
再往下      0.3877   margin 0.0102
那再来      0.3879   margin 0.0100   (borderline; ambiguous meaning, "let's redo" vs.
                                      "what's next" -- not used)
再后来呢    0.3935   margin 0.0044   (below this task's floor; not used)
那再往后    0.3937   margin 0.0042   (below floor; not used)
```

(full candidate list from both sweep rounds — ~70 more phrasings tried this
task, mostly `那...呢`/deictic-fragment analogues of the elided-argument
shape above, all leaking at 0.42–0.53 — is in this task's own report under
`.superpowers/sdd/`, not duplicated here.)

`那再后来` has the best margin but is grammatically a double continuation
marker ("so, and-then-afterward") — plausible colloquial Chinese but more
marked than the other two. `再后来` and `那后来` are both unmarked, everyday
continuation phrases with no such caveat, so those two were selected: both
verified end-to-end (not just the gate) —

```
followup-zh-01 "再后来"   raw 0.3768 (fails, margin 0.0211)
  resolved "本科毕业后他又读了什么学位？" -> 0.5355 (passes, margin 0.1376)
  retrieves pages/education.html at all 4 top-4 ranks, keyword 生物医学信息学 found

followup-zh-02 "那后来"   raw 0.3857 (fails, margin 0.0122)
  resolved "哈佛毕业后他又去哪里读书了？" -> 0.5481 (passes, margin 0.1502)
  retrieves pages/education.html at all 4 top-4 ranks, keyword 南加州大学 found
```

Both resolved forms also clear the currently-DEPLOYED zh gate's threshold
(0.4919, see the CRITICAL section at the top of this document) with margin
to spare (0.5355, 0.5481) — the rescue these two cases measure is not an
artifact of this repo's specific local threshold.

**Reading, precisely stated so it isn't overclaimed:** this is a real,
narrower phenomenon (narrative-continuation fillers), not evidence the
original "EN phenomenon" framing was wrong about the elided-argument shape
that motivates `followup-en-01`/`02` — that shape still has no zh analogue
under threshold, confirmed freshly. **These two zh cases' margins (0.01–0.02
under the LOCAL threshold) are thin** — thinner than either EN case's
(0.056/0.053) — and this document's own point about zh threshold instability
(0.3979 today, 0.4919 deployed, non-separating in between on at least one
past build) still applies: an `about_zh.md` edit that nudges the local
threshold down by even ~0.01–0.02 would flip one or both back to passing
standalone, at which point they stop measuring the rescue path.
`FOLLOWUP_POSITIVES_PER_LANG`'s own comment names this as the thing to watch
for on every future rebuild, not something to assume stays true.
